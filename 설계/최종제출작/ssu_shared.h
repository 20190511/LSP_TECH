...
#define BACKUP_USERNAME     "junhyeongBKS"
char changed_username[256];
...
void get_backuppath()
{
    if (!exist_username())
    {
        sprintf(BACKUP_PATH, "%s%s%s","/home/", BACKUP_USERNAME, "/backup");
    }
    else
    {
        find_anyuser();         //아무 home 경로에 백업폴더를 만들도록 설정.
        sprintf(BACKUP_PATH, "%s%s%s","/home/", changed_username, "/backup");
    }
    /*
...
char* find_anyuser()
{
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (strstr(home, "/home") != NULL) {
            ...
            fclose(fp);
            strcpy(changed_username, usrname);
            return usrname;
        }
    }
    fclose(fp);
