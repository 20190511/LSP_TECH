...
case1: <STD_DIR> <ANS_DIR> 존재
	1. cOption = 1 설정
	2. 인자로 받은 [STUDENTS …] 를 -t 옵션처리한것처럼, 개별적으로, t checkOpt (StdList* list, char* id); 처리
만약, 인자가 하나도 없는 경우 2 설정
		a. 실패시 즉시종료 (학번이 없는경우)
		b. 성공시 double score_student(int fd, char *id) 실행할 때, 해당 Node에 print_opts 가 있는지 확인 후 있으면 sums 출력
			i. 실제 설계시, cOption, pOption, sOption 이외에는 id_name 만 링킹되어있고, 
			ii. 자세히는 c,p 옵션에서 학번을 받는경우 5개의 학번만 StdNode가 모두 채워질 예정
	3. 해당 정보를 기준으로 score_student() 진행
	4. 종료 후 csv 저장위치 출력
	
	
case2: <STD_DIR> <ANS_DIR> 존재X : 이게 핵심이다!
	1. gets 이용해서 파일 한 줄씩 데이터를 가져옴..
	2. 2번까지 동일..
	3. -n 옵션 존재하는 경우 -n 옵션의 csv 파일 오픈..
	4. StdNode* printNode := extract_fromcsv (char* string, StdList* list, int c_opt, int p_opt) 호출 
	5. 양식에 맞게 cOption 값을 확인한 후 1 이면 노드확인, 2이면 무조건 출력
		a. printNode 값으로 양식에 맞게 출력.

<무조건 구현>
[틀린문제 + 총점수 추출함수: StdNode* extract_fromcsv (char* string, StdList* list, int c_opt, int p_opt)] //고민중..
	- return값 : 해당 학번의 StdNode* return
	- 맨 처음 학번을 찾아야함. (StdList* 에서 학번 찾기.)
	- 문자열 입력받은 것을 기준으로 score_table 을 돌면서 만점이 아닌 문제들을 뽑아내서 ssu_scorelist에 연결. (p_opt 존재시)
		○ p_opt :1 이면 print_opts 확인, p_opt: 2이면 무조건 연결
	- 마지막의 sums 값 넣기. (c_opt 존재시)
	- (20200003,0,0,0.50,0.50,0.50,0.50,0,0,0.50,0,0.50,0.50,0.50,0.50,0.50,0.50,0,0.50,0.50,0,0.50,0.50,0.50,0.50,0.50,0.50,0.50,0.50,0.50,0.50,0.50,0,0.50,0.50,0.50,0.50,0.50,0,0.50,0.50,0.50,0.50,0.50,0.50,0,0,0.50,0.50,0.50,0,0.50,0,0.50,0.50,0,0.50,0.50,0.50,0.50,0.50,0.50,0.50,0.50,0.50,0.50,0,0.50,0.50,0.50,0,1.00,1.00,1.00,1.00,1.00,0,1.00,1.00,1.00,1.00,36.00, 을 전달받으면 
...
단독사용	-h (타옵션 같이 사용시 에러)
	...
	0. 기본옵션
		...
		c. 정답 파일도 학생 답안과 동일하게 하나의 디렉토리 안에 문제번호 와 프로그램문제번호 디렉토리 문제번호 문제번호
		d. 출제되는 문제 수는 최대 문제 100 까지 허용 
		e. ssu_score은 링크드 리스트로 파일 리스트를 관리해야 함 --> (-p, -s 에서 링크드리스트 사용)
			i. (지금 당장생각나는 아이디어) : ssu_scoreTable 구조체를 
			ii. 리눅스 상에서 파일 경로의 최대 크기는 4,096 , 255 바이트이며 파일 이름의 최대 크기는 바이트임 - 
		f. 실행결과 및 에러 메시지를 파일에 출력 시 함수 system() 사용 금지 
		g. - popen(), fork(), exec() 계열 함수 사용 금지 
...
옵션 구현
	...
	주의 	1. 얘도 result saved.. (경로…) 가 있어야함 
		...
		<개인 소견>
		1. 기본 score.csv를 생성하지 말고 <CSVFILENAME>으로 생성
	설계 방식	
	2. -m
	:점수 테이블 파일 내의 특정 문제 번호의 배점 수정 (이미 구현되어있음)
	주의 	점수 테이블 파일이 존재하지 ㅇ낳을 경우 에러처리 후 프로그램 종료
	...
	주의 	 <인자 주의>
		        ...
		        B. 가변인자 개수를 5개로 제한 5개 넘을 시 
		                -->“Maximum Number of Argument Exceeded. :: [STUDENTIDS ...]” 출력 후 결과 반영 X
		        C. 첫 번째 인자로 받은 학번이 존재X || <STD_DIR> <ANS_DIR> 없이 프로그램 실행 시
		                --> 에러처리 후 프로그램 종료
		        D. <STD_DIR> <ANS_DIR> 있는데 채점파일 없으면 ( 프로세스가 종료되도록 설정)
		        E. <STD_DIR> <ANS_DIR> 두개 중 1개만 나오는 경우 에러처리
		        F. <STD_DIR> <ANS_DIR> 있는데 -n 옵션사용 --> (새로운 score.csv저장)
		        
		<개인 소견>
		        1. -n 옵션을 먼저 만들어야 제작 가능.
		        2. 파일리스트 구현에 신경써야할듯   --> -C 옵션에는 링크드리스트 안해도됨 (배열사용)
		        3. 
		                a. -c 뒤에 오는 옵션들은 stduent 구조체로 append 한다.
		                b. 형태를 보아하니, id_table 을 이용해서 
		                c. -c 뒤에 입력받는 인자 중 학번 하나라도 없을 시 프로세스 종료
		                d. student 구조체에 -p 옵션들어오면 student 구조체 멤버변수 중 ssu_scoreTable 
	...
	주의 	 <인자 주의 : -c 옵션과 동일> 
		        ...
		        A. -p -c 옵션 같이 나오면 점수 먼저 출력하고 틀린문제(점수) 출력
		        20190002 is finished.. score : 73.5, wrong problem : 1-3(3), 2-2(2), 2-3(2), 3-2(1.5), 3-3(3), 5-1(4), 8(6), 10(5) 
		        20190003 is finished.. score : 72.0, wrong problem : 1-3(3), 2-2(2), 2-3(2), 3-1(3), 3-3(3), 5-1(4), 8(6), 10(5)
		 ...
		 <추가, 중요 내용>
		 틀린문제 출력은 각 학생이 채점이 끝난 이후 수행되며, 링크드 리스트로 틀린 문제 목록을 관리해야함.
	...
	주의 	 인자 : 1 (오름차순 (작은 순))
		 ...
		 <개인소견>
		 ※ 채점 결과 파일을 <CATEGORY> 기준으로 링크드 리스트를 사용하여 정렬 후 저장.
		        -> 채점은 테이블 이용해서 하되, 채점 결과를 저장한 후 링크드리스트 손질 하라는 뜻
		                (원래 설계대로 진행)
		 파일리스트를 관리하는게 아니라, 점수관리니까 교수님 학번 정렬 코드를 살짝 손보면 되지않을까..
...
질문, 교수님의 수정명세 덕분에 굉장히 많은 도움이되었습니다 감사합니다! 추가적으로 모호한 명세질문 드립니다.
 1. -c 옵션시 학생 평균은 모든 학생의 평균인가요 아니면, 출력된 학생의 평균인가요?
	(명세에는 출력된 3 학생의 평균이 71.0점이여서 모호합니다.)
 2. ssu_score 실행 시 예외처리하는 명세 7페이지에서, "존재하지 않는 디렉토리 혹은 파일…"이라는 것은 <ANS_DIR> 에는 1.txt 답지가 있지만, 한 명의 학생이라도 학번 디렉토리에 1.txt 라는 답안지가 없는 경우에 프로그램을 종료하는 것으로 이해해도 되나요?

설계 순서 
	1. option 처리
	2. 링크드리스트 설계 (정의 함수 설계
	3. 리스트 정렬 함수 설계
	4. n 옵션 설계
	5. c, p 옵션 설계
	6. 
 ...
 1
	...
	B. <링크드 리스트 관련 질문, >
전 학번 파일리스트를 아래와같이 추출하도록 설계? (질문중 -- 대답에 따라 리스트형태는 바뀔 수 있음.)
	Ø 04.07 링크드 리스트는 -s, -p 옵션만 구현하되, -s 옵션은 전체리스트 구현, -p 옵션은 틀린문제만 구현방식으로 진행
...
// 만점 기준으로 조금이라도 점수가 깎이면 틀린 문제로 설정,
 [StdList 구조체]
	...
	2. StdNode* tail;     // 머리부분
	3. StdNode* id_cnt; // 학생 총 수
	4. int detailM         // 0: id_name 만 채움, 1: pOption, sOption 일 때 설정하고 StdNode 전 데이터를 채움
	리스트 구조 방식 	 1. -p 만 존재
		        pOption = 1 --> 틀린문제 리스트 ssu_scorelist append
		 2. -s 만 존재
		        sOption = 1 --> 전체 리스트 append.
		 3. -p, -s 둘 다 존재 --> 예외처리 (리스트 충돌방지)
 ...
 [StdNode 구조체]
	...
	2. double sums    //학생 총점
	3. int print_opts      //  0:미지정, 1:지정. --> 지정되면 아래 wrong_cnt, 리스트 사용.
	--> 전역변수 char idArgs[ARGNUM][10]; 이용 예정,
	4. int wrong_cnt   // 총 틀린개수.
	5. StdNode* next;   // 다음 학생노드
	6. ssu_scorelist* list_header;   // header->..->..->tail->NULL;
	7. ssu_scorelist* list_tail;



[ssu_scorelist]
char qname[FILELEN];    // 문제번호
int cur_score;          // 현 점수 (sOption 전용)
int score;              // 원래 배점 (pOtion 전용)
ssu_scorelist* next;    // 다음 노드




링크드 리스트 관리함수 (StdNode append도 필요함.)
int append (StdList* list, char* pathname);          // pathname으로 노드를 할당해서 연결
int append_score (StdNode* stdnode, char* qname, int cur_score);  // stdnode 에 {문제번호, 현재점수, 원래점수 연결} 
ssu_scorelist* append_score (StdNode* stdnode);   //(혹시 모르니까), 구현
int append_node (StdList* list, StdNode* node);   // Node 를 할당하지 않고, 기존 노드 연결
StdNode* nofree_pop(StdList* list);   // 맨 앞 요소를 삭제하고, 해당 노드 반환 --> s옵션에서 리스트 재구성할 때 사용,
void pop(StdList* list);    // free 해주면서 pop
void print_node(StdList* list);
void swap_node (StdNode* p, StdNode* a, StdNode* b);
int checkOpt (StdList* list, char* id);     //list 링크드리스트에 일치하는 id 있으면 해당노드 print_opts=1; 처리,
...
만약, 인자가 하나도 없는 경우 2 설정
		a. 실패시 즉시종료 (학번이 없는경우)
		b. 성공시 double score_student(int fd, char *id) 실행할 때, id_
		c. 해당 Node에 print_opts 가 있는지 확인 후 있으면 sums 출력
			i. 실제 설계시, cOption, pOption, sOption 이외에는 id_name 만 링킹되어있고, 
			ii. 자세히는 c,p 옵션에서 학번을 받는경우 5개의 학번만 StdNode가 모두 채워질 예정
	3. 해당 정보를 기준으로 score_student() 진행
	4. 종료 후 csv 저장위치 출력
...
case2: <STD_DIR> <ANS_DIR> 존재X : 이게 핵심이다!  <--- 명세 삭제!!!
	1. gets 이용해서 파일 한 줄씩 데이터를 가져옴..
	2. 2번까지 동일..
	3. -n 옵션 존재하는 경우 -n 옵션의 csv 파일 오픈..
	4. StdNode* printNode := extract_fromcsv (char* string, StdList* list, int c_opt, int p_opt) 호출 
...
[find csv]
	...
	- p 옵션 설계
• pOption = 1 설정
• 인자로 받은 [STUDENTS …] 를 -t 옵션처리한것처럼, 개별적으로, t checkOpt (StdList* list, char* id); 처리
• student_score() 내부에서 p 옵션 시, 


[print_wrongnum (Stdnode* node)]
if (node == NULL)
	return;
printf("wrong problem : ");
for (Qname := node->list_header ; Qname != NULL ; Qname = Qname->next)
	printf("%s(%s)", Qname->qname, Qname->score);
	if (Qname->next !=NULL)
		printf(", ");
printf("\n");
...
-s 옵션 설계 (score_table도 리스트로 만들어야되면, 손봐야함) --> 링크드리스트 기반 (틀린문제만 정렬하지 말고, 전체정렬해야함!)
 --> 리스트 정렬함수 제작 (student_score_S(), student_score_S()) 내부에서 진행..
		a. --> 인자로 링크드 리스트 전달 추가..?
	1. student_score() 진행하면서 리스트 append
	2. 아래 내부함수 이용해서 리스트 재정렬
	3. 정렬이 끝났으면 writeSort() 호출
...
[링크드리스트 swap 방식 구현]
1. 버블정렬한다.
2. 바꿀 B,C 와 부모노드(A) 를 for문 상에서 기억해놓는다.
[swaping함수 swap_list (StdNode* parent, StdNode* a, StdNode* b);
	1. B->next = C->next
	2. A->next = C;
	3. C->next = B;
3. 해당 정렬을 진행한다.
...
[출력 함수 : writeSort(StdList* Stdlist, int fd)]
char tmp [20];
write_first_row ()
idName := StdList->list_header;
for ( ; idName != NULL ; idName = idName->next)
	sprintf(tmp, "%s,", idName->id_name); // tmp = 2020XXXX
	write(fp, tmp, sizelen(tmp);
	Qname := idName->list_header;
	for ( ; Qname != NULL ; Qname->next)
...
![image](https://user-images.githubusercontent.com/70988272/230616993-db694d04-a469-4820-bc1c-7bf33805e045.png)
