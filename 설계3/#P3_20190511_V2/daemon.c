...
int main(int argc, char* argv[])
{
    ...
    *ptr++ = '/';
    ...
    while(1) 
    {
        ...
        if (log_cnt % 10 == 0)
        {
            syslog(LOG_INFO, "my pid is [%05d], it's running!", getpid());
            closelog();
        }
        ...
        {
            char db[4096];
            sprintf(db, "cur(%s) head->next : %p\n",list->head->path, list->head->next);
            fprint_process(DEFAULT_OPT, log_path, db);
            //printL(list);
        }
#endif
...
int final_module (LogpathList *list, char* path, unsigned int timer) 
{
    ...
    // final_moudle 과정에서 head를 손본게 문제가 생길 수 있어 넣어준 문장
    if (list->tail != NULL && (list->head == NULL || list->head->prev != NULL))
    {
        list->head = list->tail;
...
int compare_list (LogpathList *list, char* path)   
{
    ...
    *ptr = '\0';
            ...
            else if (S_ISREG(statbuf.st_mode))
            {
                if(!compare_node(list, tmp_path))
                {
                    ...
                    if (strcmp(tmp_path, monitor_path) && strcmp(tmp_path, log_path) && exclusive_swpfile(tmp_path))
                    {
                        ...
                        printL(list);
#endif
                        if (list->cnt == 1)  // 헤드 복귀 과정에서의 에러가 발생함.
                            saved_head = list->head;
    ...
    if (saved_head == NULL || saved_head->prev != NULL)
    {
        if (list->cnt != 0)
        {
            saved_head = list->tail;
...
void printL (LogpathList *list)
{
    ...
    for (cnt_idx = 0 ; tmp != NULL ; tmp = tmp->next, cnt_idx++)
    {
        ...
        char tmp2[4096];
        snprintf(tmp2, 5200, "[%-4d] : %-70s   --> (%s)\n", cnt_idx+1, tmp->path, buf);
        fprint_process(DEFAULT_OPT, log_path, tmp2);
    }
}
...
LogPath* appendL (LogpathList *list, char* path)
{
    ...
    if (list->head == NULL && list->cnt == 0) //개수도 0이여야함!
    {
