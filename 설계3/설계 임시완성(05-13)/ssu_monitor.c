...
void dyingMessage(); //디몬프로세스로부터 전달받은 죽으면서 출력한 다잉메시지 받아보는 함수.
...
char daemon_msg_path [MAXPATHLEN];
...
void dyingMessage() //디몬프로세스로부터 전달받은 죽으면서 출력한 다잉메시지 받아보는 함수.
{
    ...
    //sleep(1);
    FILE* die_fp = fopen(daemon_msg_path, "r");
    char buf [BUFSIZ] = {0,};
    while (strlen(buf) == 0) // 버퍼를 읽어들일 때 까지 동기화
    {
        fgets(buf, BUFSIZ, die_fp);
        fseek(die_fp, 0, SEEK_SET);
    }
    printf("%s", buf);
    fclose(die_fp);
}
...
int path_setting ()
{
    ...
    snprintf(daemon_msg_path, MAXPATHLEN+10, "%s/die.txt", current_path);
    return true;
}
